...
for num_modals in modals_arr:
	...
	modals = fock_object.M * [num_modals]
	...
	my_r = r_arr[0]
	if my_r is None:
		rpt = r_from_pt(fock_object, dip_arr, modals, Hnorm, frag_method = frag_method, epsilon_in_cm=epsilon_in_cm, verbose=False, mol_name = mol_name, is_loc = is_loc, num_couplings = num_couplings, frag_tol = frag_tol)
		if rank == 0:
	...
	num_qubits, pl_ham = vib_from_modes(Hvscf, modals, ps=True)
	...
	for sim_num in range(num_rs):
		...
		#{ Added by SVM 
		trot_calls, tot_frags, rz_per_trot, rz_count = count_steps_fixed_time(pl_ham, my_r, my_kmax, trotter_order= trotter_order, frag_method = frag_method, num_couplings = num_couplings, is_loc = is_loc ,mol_name = mol_name, modals = modals, frag_tol = frag_tol)
